...
export default function RideCard({ ride, currentUser, onRequestSeat, onRespondRequest, onSendMessage, onDelete, onCancelRequest }) {
  ...
  const getCountdown = () => {
    ...
    if (hrs > 48) return null; // Don't show for far-off rides
  ...
  return (
    <div className={`bg-white/30 backdrop-blur-xl rounded-2xl shadow-lg border border-white/50 flex flex-col h-full overflow-hidden ${userStatus === 'accepted' ? 'shadow-emerald-500/20 bg-emerald-50/40' : 'card-hover'}`}>
      {/* Countdown Badge */}
      {countdown && (
          ...
          <div className="w-full flex flex-col gap-2">
            <div className="w-full flex items-center gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 group transition-all py-2.5 rounded-xl font-bold ${userStatus === 'pending' ? 'bg-amber-100 text-amber-700 cursor-not-allowed border border-amber-200' :
              ...
              {(userStatus === 'pending' || userStatus === 'accepted') && (
                <button
                  ...
                  className="p-2.5 bg-red-100 text-red-600 hover:bg-red-200 hover:text-red-700 rounded-xl transition-colors border border-red-200 flex-shrink-0"
